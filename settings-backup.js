...
export function exportSettings(root) {
  ...
  return {format:'dico-settings',version:1,createdAt:new Date().toISOString(),channels:root.channels.map(c=>({
    name:c.name,messages:[...c.messages],intervalSeconds:c.intervalSeconds,ownUserId:c.ownUserId,
...
export function importSettings(data) {
  ...
  return data.channels.map(c=>{
    ...
    return {name:c.name.trim(),messages:[...c.messages],intervalSeconds:c.intervalSeconds,ownUserId:c.ownUserId||'',nextIndex:c.nextIndex,
