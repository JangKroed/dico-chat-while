...
testButton.addEventListener('click', async () => {
  ...
  try {
    const result=await withNotificationTimeout(createDiagnosticNotification(chrome,'dico-error:test', {
    ...
    status.textContent = (result.registered ? '브라우저가 테스트 알림 요청을 받았습니다. 알림 목록 등록도 확인했습니다. ' : '브라우저가 요청을 받았지만 활성 알림 목록에는 없습니다. ') + ' 화면에 보이지 않으면 알림 센터, Chrome/Brave 알림 허용, 집중 모드·방해금지 설정을 확인하세요. Discord 채팅으로 보내는 알림은 아닙니다.';
