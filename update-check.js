...
export function installUpdateChecks(api){
 ...
 const setup=async()=>{if(await api.permissions.contains({origins:[UPDATE_ORIGIN]})){await api.alarms.create('dico-update-check',{periodInMinutes:60});await poll();}};
 ...
 api.permissions.onRemoved.addListener(()=>{void api.permissions.contains({origins:[UPDATE_ORIGIN]}).then(allowed=>{if(!allowed)return api.alarms.clear('dico-update-check');}).catch(()=>{});});
 void setup().catch(()=>{});
}
